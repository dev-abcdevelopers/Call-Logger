...
interface ChartProps {
  type: ChartType
  data: ChartJSData<ChartType>
  options?: ChartJSOptions<ChartType>
  height?: number
  width?: number
  className?: string
}
...
export function Chart({ type, data, options, height, width, className }: ChartProps) {
  const defaultOptions: ChartJSOptions<ChartType> = {
    responsive: true,
    plugins: {
      legend: {
  ...
  const renderChart = () => {
    switch (type) {
      ...
      case "doughnut":
        return (
          <Doughnut
            data={data as ChartJSData<"doughnut">}
